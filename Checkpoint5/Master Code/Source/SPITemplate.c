...
static uint32_t ADResults[4];			// Array to save the ADC results
...
void StartDRS( ES_Event CurrentEvent )
{
	...
	// Set aforementioned pins as outputs
...
static bool DRSSaveData ( void)
{
	...
	if( (NewDRSRead[1] & NewDRSRead[2] & NewDRSRead[3] & NewDRSRead[4] & NewDRSRead[5] & NewDRSRead[6] & NewDRSRead[7]) != INVALID_READ )
	{
		...
		switch(CurrentQuery)
		{
			case QUERY_GAME_STATE  :
			{
				...
				//printf("   GameState: %d LapsRemaining: %d ShotMade: %d obstacle: %d\r\n", Kart1.KartX, Kart1.KartY, QueryTheta());
			...
			case QUERY_KART1 : 
			{
				...
				//printf("   KARTX: %d KARTY: %d KARTTheta: %d\r\n", Kart1.KartX, Kart1.KartY, QueryTheta());
				break;
			}
	...
	// Calculate our own theta based on the deltaX deltaY
	// Find distance to travel
	
	/*
	if(((LastX && LastY) != 0) QueryCount)
	{
		float deltaX = LastX - CurrentKartState.KartX;
		float deltaY = LastY - myKart.KartY; 
...
static void CheckDRSEvents( void )
{
	//printf("Current GameState: %d\r\n", CurrentKartState.GameState);
	if(CurrentKartState.GameState != LastKartState.GameState)
	{
		//printf("DRS Event Detected\r\n");
		ES_Event NewEvent;
