...
ES_Event RunMaster( ES_Event CurrentEvent )
{
    ...
    */
	...
	if(CurrentEvent.EventType == AtNextPoint || CurrentEvent.EventType == CautionFlagDropped || CurrentEvent.EventType == EmergencyStop || CurrentEvent.EventType == GameOver) {
		PostDrive(CurrentEvent);
	}
