...
POINT_t nearLinePointShooting = {500,100};
POINT_t nearLinePointObstacle = {200,700};
...
ES_Event RunRunningGame( ES_Event CurrentEvent )
{
   ...
   switch ( CurrentState )
   {
	...
	/*add new exit event from the shootingSM 
			 it should indicate that we have returned to a point near the tape line*/
       ...
       case ObstacleStateSM :
         ...
         CurrentEvent = DuringObstacleStateSM(CurrentEvent);
			 ...
			 /*add new exit event from the shootingSM 
			 it should indicate that we have returned 
			 to a point near the tape line*/
