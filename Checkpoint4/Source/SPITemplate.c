...
#define DRS_COMMAND_TIMEOUT 1000		// Tick count for SendCommand timeout (10 ms)
...
static KART_t CurrentKartState;			// Structure to save our Kart
static KART_t LastKartState;				// Structure to save our Kart

static uint16_t NewDRSRead[8]; 			// Array to save 8 byte response from DRS
static uint16_t Kart1ThetaArray[5];		// Array to smooth theta for Kart1
static uint16_t Kart2ThetaAverage[5];		// Array to smooth theta for Kart2
static uint16_t Kart3ThetaAverage[5];		// Array to smooth theta for Kart3
static int Kart1ThetaArraySize = 0;
static int Kart2ThetaArraySize = 0;
static int Kart3ThetaArraySize = 0;
	...
	if( (HWREG(SSI0_BASE + SSI_O_SR) & SSI_SR_BSY) != SSI_SR_BSY )
	{
		...
		if( (HWREG(SSI0_BASE + SSI_O_SR) & SSI_SR_RFF) == SSI_SR_RFF )
		{
			...
			for (Index = 0; Index<8; Index++)
			{
				...
				//printf("ThisRead[%d]: %d\n\r", Index, ThisRead);
				// Set NewDRSRead at index to the most significant 8bits of ThisRead
				// since this is the 8bit response from the DRS
				NewDRSRead[Index] = ThisRead;
				// Print out the value for debugging purposes
				//printf("NewDRSRead[%d]: %d\n\r", Index, ThisRead);
			}	
		}
	}
	...
	if( (NewDRSRead[1] & NewDRSRead[2] & NewDRSRead[3] & NewDRSRead[4] & NewDRSRead[5] & NewDRSRead[6] & NewDRSRead[7]) != INVALID_READ )
	{
		...
		switch(CurrentQuery)
		{
			...
			case QUERY_KART1 : 
			{
				...
				uint16_t DRSTheta = Kart1.KartTheta;
				
				if(Kart1.KartTheta > 350 || Kart1.KartTheta < 10){}
					// don't do anything
				else
				{
					...
					for(int i = 4; i > 0; i--)	
					{
						Kart1ThetaArray[i] = Kart1ThetaArray[i-1];
						MovingAverage += Kart1ThetaArray[i-1];
						//printf("MovingAverage: %d \r\n", MovingAverage);
					}
					Kart1ThetaArray[0] = Kart1.KartTheta;
					MovingAverage += Kart1ThetaArray[0];
					//printf("MovingAverage: %d \r\n", MovingAverage);
					Kart1.KartTheta  = MovingAverage/5;
					
				}
			
				// No more data to save, print the new KART information
				//printf("KART1 position/orientation received:\r\n");
				//printf("   KARTX: %d KARTY: %d FILTERTheta: %d DRSTheta:  %d\r\n", Kart1.KartX, Kart1.KartY, Kart1.KartTheta, DRSTheta);
				break;
			}
			...
			case QUERY_KART2 :
			{
				...
				if(Kart2.KartTheta > 350 || Kart2.KartTheta < 10){}
					// don't do anything
					
					// Reset Kart1ThetaAverage
//					
//					// Add one value to the first element of the array
//					Kart1ThetaArray[0] = Kart1.KartTheta;
//					Kart1ThetaArraySize = 1;
//					
//				
				else
				{
					...
					for(int i = 1; i <10; i++)	
					{
						Kart2ThetaAverage[i] = Kart2ThetaAverage[i-1];
						MovingAverage += Kart2ThetaAverage[i-1];
					}
					Kart2ThetaAverage[0] = Kart2.KartTheta;
					MovingAverage += Kart2ThetaAverage[0];
					Kart2.KartTheta  = MovingAverage/10;
					
				}
				
				
				// No more data to save, print the new KART information
				//printf("   KART2X:  %d   KART2Y:  %d    KART2Theta:  %d\r\n", Kart2.KartX, Kart2.KartY, Kart2.KartTheta);
			...
			case QUERY_KART3 :
			{
				...
				if(Kart3.KartTheta > 350 || Kart3.KartTheta < 10){}
					// don't do anything
				else
				{
					// Smooth with a moving average
					...
					for(int i = 1; i <10; i++)	
					{
						Kart3ThetaAverage[i] = Kart3ThetaAverage[i-1];
						MovingAverage += Kart3ThetaAverage[i-1];
					}
					Kart3ThetaAverage[0] = Kart3.KartTheta;
					MovingAverage += Kart3ThetaAverage[0];
					Kart3.KartTheta  = MovingAverage/10;
					
				}
				
				// No more data to save, print the new KART information
				//printf("   KART3X:  %d   KART3Y:  %d    KART3Theta:  %d\r\n", Kart3.KartX, Kart3.KartY, Kart3.KartTheta);
				break;
			}
		}
	}
